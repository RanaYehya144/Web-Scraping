...
def articles_by_author(author_name):
    try:
        ...
        result = list(collection.aggregate(pipeline))
        if not result:
            return jsonify({"error": "No articles found for author"}), 404
        return jsonify(result)
...
@app.route("/article_details/<string:post_id>", methods=["GET"])
def article_details(post_id):
    print(f"Received post_id: {post_id}")  # For debugging purposes

    pipeline = [
        {"$match": {"post_id": post_id}},  # Match using post_id as a string
        {
            "$project": {
                "_id": 0,
                "url": 1,
    ...
    if not result:
        return jsonify({"message": "No article found with the given post_id"}), 404
